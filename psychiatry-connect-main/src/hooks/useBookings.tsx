      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          ...
          recordings(id, recording_url, expiry_date, status, created_at)
        `)
...
export function useCreateBooking() {
  ...
  return useMutation({
    mutationFn: async (booking: CreateBookingData) => {
      ...
      if (error) throw error;
      const bookingResult = transformBookingData(data) as Booking;

      // Send trainer notification (fire-and-forget)
      try {
        ...
        if (trainerData?.email) {
          supabase.functions.invoke("notify-trainer-booking", {
            body: {
              ...
              candidate_name: profile?.full_name || user.email || "A candidate",
