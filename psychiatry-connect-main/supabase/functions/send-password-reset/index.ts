...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Ensure the action link uses the direct Supabase auth URL, not any proxy
    // Replace any Lovable preview proxy URL with the actual Supabase auth URL
    const supabaseAuthBase = `${SUPABASE_URL}/auth/v1`;
